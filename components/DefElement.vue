...
const unparsed = computed(() => {
        ...
        return props.body.items.map(
          function(item){
            ...
            else if (item.id) { console.log("item", item.id) ; return {type: item.type_, html:  ({"nn":store.concepts_nn, "bm":store.concepts_bm}[props.dict].data.concepts[item.id] || {})['expansion'] || item.id} }
