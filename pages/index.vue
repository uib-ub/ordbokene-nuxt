...
<main id="main" tabindex="-1" class="flex flex-col grow-1 relative">
  ...
  <SearchNav v-if="route.name != 'search'"/>
    ...
    <div v-if="route.name == 'article' && store.searchUrl" class="ord-container justify-start mt-2">
      <NuxtLink :to="store.searchUrl"> <BootstrapIcon name="arrow-left-short" class="mb-1 text-primary text-lg"/>{{$t('notifications.back')}}</NuxtLink>
    </div>
    <div v-else-if="route.name == 'article'" class="ord-container justify-start mt-2">
      <NuxtLink :to="`/${$i18n.locale}/`"> <BootstrapIcon name="arrow-left-short" class="mb-1 text-primary text-lg"/>{{$t('home')}}</NuxtLink>
    </div>
  </NuxtErrorBoundary>
